...
const AddNewStudent = (props) => {
  ...
  // Auto-populate school code, school name, teacher code, and teacher name
  useEffect(() => {
    if (userRole === "SCHOOL_ADMIN" && userDetails) {
      setUserData(current => ({
        ...current,
    ...
    if (userRole === "TEACHER" && userDetails) {
      setUserData(current => ({
        ...current,
        schoolCode: userDetails.school_code || '',
  ...
  const validateForm = () => {
    ...
    } else if (userData.rollNumber.length > 10) {
    ...
    if (userRole === "SUPER_ADMIN") {
      ...
      if (!userData.teacherCode || userData.teacherCode.trim().length === 0) {
        newErrors.teacherCode = "Teacher Code is required";
      }
    }
    
    // School Admin must provide teacher code
    ...
    if (userData.fatherName && userData.fatherName.length > 40) {
          ...
          <div className="addTextfieldContainer">
            ...
            <div className="AddStudentTextFieldContainer">
              ...
              <MSTextField
                ...
                helperText={errors.fatherName || "Max 40 characters, alphabets only"}
              />
            </div>
            <div className="AddStudentTextFieldContainer">
              <MSTextField
