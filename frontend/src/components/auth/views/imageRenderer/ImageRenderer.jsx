...
const ImageRenderer = (props) => {
  ...
  useEffect(() => {
    ...
    counter.current = 0;
    setPage(1);
    // Calculate page count based on totalCount or filtered data
    if (props.pageType === 'result') {
      setPageCount(pagination)
    } else if (props.totalCount && props.totalCount > 0) {
      // Use server-provided total count for accurate pagination
      setPageCount(Math.ceil(props.totalCount / 21))
    } else {
      // Fallback: Filter for displayable files (images and PDFs)
  ...
  const renderImages = () => {
    ...
    if ((!oldData || (Array.isArray(oldData) && oldData.length === 0) || Object.keys(oldData || {}).length === 0) && props.loading) {
      return (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '300px', gap: '16px', padding: '40px', width: '100%' }}>
  ...
  const downloadImage = async () => {
    ...
    const deleteImage = () => {
      ...
      // Allow delete for MyImages regardless of exact path
      if (props.place === "MyImages" || location.pathname === "/auth/images" || location.pathname.includes("/images")) {
        let header = {
          "Content-Type": "application/json",
        ...
        displaySnackbar({ message: "Delete is only available in My Images" });
      }
    };
