...
const MicroScheduler = () => {
  ...
  // Get subject for a specific date (for Month/Year views)
  ...
  // Get ALL subjects for a specific date (for Month view multi-subject display)
  const getAllSubjectsForDate = (dateKey) => {
    const daySchedule = weeklySchedule[dateKey];
    if (!daySchedule) return [];
    // Return all unique subjects for this date
    const periodKeys = Object.keys(daySchedule).sort();
    const subjectsMap = new Map();
    periodKeys.forEach(periodKey => {
      const subject = daySchedule[periodKey];
      if (subject) {
  ...
  // Get subject count for a date (for displaying "+N more" indicator)
  const getSubjectCountForDate = (dateKey) => {
    const subjects = getAllSubjectsForDate(dateKey);
  ...
  const handleDeleteSubject = (subjectId) => {
    // Find subject name for confirmation message
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 1 }}>
          ...
          {weeks.map((week, wIdx) => (
            week.map((dayInfo, dIdx) => {
              ...
              // Get ALL subjects for this date (FIX for Issue #94)
              const allSubjects = dateKey ? getAllSubjectsForDate(dateKey) : [];
              const hasSubjects = allSubjects.length > 0;
              const firstSubject = allSubjects[0] || null;
              ...
              return (
                <Box key={`${wIdx}-${dIdx}`} onDragOver={handleDragOver} onDrop={() => dayNum && handleDropCalendar(dayMonth, dayNum)}
                  ...
                  sx={{ p: 1, minHeight: 60, border: '1px solid #e0e0e0', borderRadius: 1,
                    backgroundColor: hasSubjects ? firstSubject.color : (dayNum ? (isCurrentMonth ? '#fff' : '#f9f9f9') : '#f5f5f5'),
                    cursor: dayNum ? 'pointer' : 'default', opacity: isCurrentMonth ? 1 : 0.6,
                    '&:hover': dayNum ? { boxShadow: 2 } : {},
                    position: 'relative' }}>
                  {dayNum && (
                    <>
                      <Typography variant="caption" sx={{ fontWeight: 'bold', color: hasSubjects ? '#fff' : (isCurrentMonth ? '#333' : '#999') }}>{dayNum}</Typography>
                      {hasSubjects && (
                        <>
                          <Typography variant="caption" display="block" sx={{ color: '#fff', fontSize: '0.65rem' }}>{firstSubject.shortName}</Typography>
                          {subjectCount > 1 && (
                            <Typography variant="caption" display="block" sx={{ color: '#fff', fontSize: '0.55rem', fontWeight: 'bold', mt: 0.25 }}>
                              +{subjectCount - 1} more
                            </Typography>
                          )}
                        </>
  ...
  return (
    <Box sx={{ display: 'flex', height: 'calc(100vh - 120px)', bgcolor: '#f5f5f5' }}>
            ...
            <>
              ...
              {subjects.map(subject => (
                <Box key={subject.id} draggable onDragStart={() => handleDragStart(subject)}
                  ...
                  <IconButton size="small" sx={{ color: '#fff', p: 0.5, ml: 0.5 }}
                    onClick={(e) => { 
                      e.preventDefault();
                      e.stopPropagation();
      ...
      <Dialog open={subjectDialog.open} onClose={() => setSubjectDialog({ open: false, mode: 'add', subject: null })} maxWidth="sm" fullWidth>
        ...
        <DialogContent sx={{ maxHeight: '60vh', overflow: 'auto' }}>
          <TextField autoFocus margin="dense" label="Subject Name" fullWidth value={newSubject.name}
            onChange={(e) => setNewSubject({ ...newSubject, name: e.target.value })} sx={{ mb: 2 }} />
      ...
      <Dialog open={periodDialog.open} onClose={() => setPeriodDialog({ open: false, mode: 'add', period: null })} maxWidth="sm" fullWidth>
        <DialogTitle>{periodDialog.mode === 'add' ? 'Add Period' : 'Edit Period'}</DialogTitle>
        <DialogContent sx={{ maxHeight: '60vh', overflow: 'auto' }}>
          <TextField autoFocus margin="dense" label="Period Name" fullWidth value={newPeriod.name}
            onChange={(e) => setNewPeriod({ ...newPeriod, name: e.target.value })} sx={{ mb: 2 }} />
          <Grid container spacing={2}>
            <Grid item xs={6}>
