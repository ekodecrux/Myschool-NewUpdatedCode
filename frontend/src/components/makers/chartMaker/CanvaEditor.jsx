  ...
  useEffect(() => {
    ...
    if (leftTab === 2 && imageBankCategory) {
      (async () => {
        ...
        try {
          const response = await fetch(`/api/rest/images/fetch`, {
            method: 'POST',
    ...
    >
      ...
      <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <Paper sx={{ p: 1, borderRadius: 0, display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
          ...
          <Button variant="outlined" startIcon={<SaveIcon />} onClick={() => { setEditingDesign(null); setDesignName(''); setSaveDialog(true); }}>
            Save
          </Button>
          <Button 
            variant="contained" 
      ...
      <Dialog open={saveDialog} onClose={() => { setSaveDialog(false); setEditingDesign(null); }}>
        ...
        <DialogActions>
          ...
          <Button onClick={handleSaveDesign} variant="contained">Save</Button>
        </DialogActions>
      </Dialog>
      {/* Snackbar */}
      <Snackbar
