...
const Navbar = (props) => {
  ...
  // For Academic menu items (Classes, etc.), always use 'academic' as base path
  const getNavigationBasePath = (parentTitle) => {
    // If the parent menu is 'CLASS' or similar academic items, always use 'academic'
    const academicMenuItems = ['class', 'kindergarten', 'more'];
    if (parentTitle && academicMenuItems.includes(parentTitle.toLowerCase())) {
      return 'academic';
    }
    ...
    if (validBasePaths.includes(currentPath)) {
      // If on sections page but clicking academic items, switch to academic
      if (currentPath === 'sections') {
        return 'academic';  // Default academic items to academic path
      }
      return currentPath;
    }
                            ...
                            <NavLink
                              ...
                              to={{
                                    pathname: (() => {
                                      ...
                                      const currentBasePath = getNavigationBasePath(k.title);
