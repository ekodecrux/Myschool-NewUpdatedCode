...
const AdminPanel = () => {
  ...
  return (
    <Box sx={{ p: { xs: 1, sm: 2, md: 3 }, bgcolor: '#f5f5f5', minHeight: '100vh' }}>
      <Typography variant="h4" gutterBottom sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap', fontSize: { xs: '1.25rem', sm: '1.5rem', md: '2rem' } }}>
        🎛️ Admin Panel
      ...
      <Paper sx={{ width: '100%', overflow: 'hidden' }}>
        ...
        >
          ...
          {userRole === 'SUPER_ADMIN' && <Tab icon={<ImageIcon sx={{ fontSize: { xs: 16, sm: 20 } }} />} label={`Images (${pendingImages.length})`} />}
          {userRole === 'SUPER_ADMIN' && <Tab icon={<CreditCardIcon sx={{ fontSize: { xs: 16, sm: 20 } }} />} label={`Credits (${creditRequests.length})`} />}
          {userRole === 'SUPER_ADMIN' && <Tab icon={<LogsIcon sx={{ fontSize: { xs: 16, sm: 20 } }} />} label="User Logs" />}
          {userRole === 'SUPER_ADMIN' && <Tab icon={<SalesIcon sx={{ fontSize: { xs: 16, sm: 20 } }} />} label="Sales Plans" />}
        ...
        {userRole === 'SUPER_ADMIN' && (
          <TabPanel value={tabValue} index={0}>
            <AnalyticsTab accessToken={accessToken} userRole={userRole} />
          </TabPanel>
        )}
        ...
        {userRole === 'SUPER_ADMIN' && (
          <TabPanel value={tabValue} index={5}>
            <UserLogsTab accessToken={accessToken} userRole={userRole} />
          </TabPanel>
        )}

        {userRole === 'SUPER_ADMIN' && (
          <TabPanel value={tabValue} index={6}>
            <SalesPlanTab accessToken={accessToken} />
          </TabPanel>
        )}
