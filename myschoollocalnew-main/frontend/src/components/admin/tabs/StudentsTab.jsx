...
const StudentsTab = ({ 
  ...
  // FIX for Issue #113: Create school lookup map for resolving school names
  const schoolMap = React.useMemo(() => {
    const map = {};
    schools.forEach(school => {
      if (school.code || school.school_code) {
        map[school.code || school.school_code] = school.name || school.school_name || school.code;
      }
    });
    return map;
  }, [schools]);
  
  // FIX for Issue #113: Helper to get school name from code
  const getSchoolName = (student) => {
    if (student.schoolName || student.school_name) {
      return student.schoolName || student.school_name;
    }
    const schoolCode = student.schoolCode || student.school_code;
    return schoolCode ? (schoolMap[schoolCode] || schoolCode) : '-';
  };
          ...
          <TableBody>
            {displayStudents.map((student) => (
              ...
              >
                ...
                {/* FIX for Issue #113: Use helper to resolve school name */}
                <TableCell>{getSchoolName(student)}</TableCell>
                <TableCell>
