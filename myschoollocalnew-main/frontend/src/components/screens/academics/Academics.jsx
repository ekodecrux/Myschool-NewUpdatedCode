...
const Academics = (props, state) => {
    const { list, continuationToken, loading, filters: apiFilters, filterField, totalCount } = useSelector(state => state.api)
    ...
    const getAcademicPath = () => {
        ...
        if (firstPart === 'imagebank' || firstPart === 'image-bank') {
            if (menuParts.length > 1) {
                const category = menuParts[1].replace(/-/g, ' ').toUpperCase();
    ...
    // Check if we're on image bank page
    ...
    return (
        <>
            ...
            <div >
                {/* AcademicFilter removed - filtering is done via header menu */}
                
                {/* Blue Pane Level 2 Filter for Academic Image Bank */}
