...
const Profile = () => {
  ...
  const [credits, setCredits] = React.useState(null); // FIX for Issue #102: Use checkCredits API
  const [isUnlimited, setIsUnlimited] = React.useState(false);
  const { displaySnackbar } = useSnackbar();
  
  // FIX for Issue #102: Fetch credits from same API as TopNavbar
  useEffect(() => {
    ...
    <div className="profileManinContainer">
      ...
      <div className="myProfileContainer">
        <div className="profileWrapperContainer">
          ...
          {/* FIX for Issue #102: Use credits from checkCredits API for consistency */}
          {!isUnlimited && credits !== null ? <Card
            elevation={0}
          ...
          >
            <div style={{display: 'flex', alignItems: 'center', gap: 20}}>
              ...
              <Typography fontSize={28} color={credits <= 10 ? 'error' : 'primary'}>
                {credits}
              </Typography>
            </div>
            {credits <= 50 && (
